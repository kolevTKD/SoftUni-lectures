...
UPDATE Tickets
SET DateOfDeparture = DATEADD(dd, 7, DateOfDeparture)
  , DateOfArrival = DATEADD(dd, 7, DateOfArrival)
...
SELECT t.[Name] AS Town
  ...
  FROM Towns AS t
  LEFT JOIN RailwayStations AS rs ON t.Id = rs.TownId
...
SELECT
    ...
    rs.[Name] AS RailwayStation
FROM Towns T
JOIN RailwayStations rs ON t.Id = rs.TownID
--JOIN TrainsRailwayStations AS trs ON rs.Id = trs.RailwayStationId
WHERE NOT EXISTS (
    SELECT 1
    FROM Trains tr
    WHERE tr.ArrivalTownId = t.Id
)
...
WHERE
    NOT EXISTS (
        ...
        JOIN
            Trains tr ON trs.TrainId = tr.Id
        WHERE
            trs.RailwayStationId = rs.Id
    )
ORDER BY
...
WHERE tr.HourOfDeparture >= '08:00' AND tr.HourOfDeparture < '09:00'
  AND ti.Price > 50.00
...
SELECT @trainsCount = COUNT(DISTINCT Id)
FROM Trains
